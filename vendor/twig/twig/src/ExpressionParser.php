...
class ExpressionParser
{
    ...
    private function getPrimary(): AbstractExpression
    {
        ...
        if ($this->isUnary($token)) {
            ...
            $this->parser->getStream()->expect(/* Token::PUNCTUATION_TYPE */ 9,
                                                                             ')',
    ...
    public function parsePrimaryExpression()
    {
        ...
        switch ($token->getType()) {
                ...
                if (isset($this->unaryOperators[$token->getValue()])) {
                    ...
                    if (!\in_array($class, [NegUnary::class, PosUnary::class])) {
                        throw new SyntaxError(
                            sprintf('Unexpected unary operator "%s".', $token->getValue()),
                            $token->getLine(),
            ...
            default:
                if ($token->test(/* Token::PUNCTUATION_TYPE */ 9, '[')) {
                ...
                } elseif ($token->test(/* Token::OPERATOR_TYPE */ 8, '=')
                    && ('==' === $this->parser->getStream()->look(
                            -1
                        )->getValue()
                    ...
                    throw new SyntaxError(
                        sprintf(
                            'Unexpected operator of value "%s". Did you try to use "===" or "!==" for strict comparison? Use "is same as(value)" instead.',
                            $token->getValue()
                        ), $token->getLine(), $this->parser->getStream()->getSourceContext()
                    );
                } else {
                    throw new SyntaxError(
                        sprintf(
    ...
    public function parseHashExpression()
    {
        ...
        while (!$stream->test(/* Token::PUNCTUATION_TYPE */ 9, '}')) {
            ...
            if ($token = $stream->nextIf(/* Token::NAME_TYPE */ 5)) {
            ...
            } elseif (($token = $stream->nextIf(/* Token::STRING_TYPE */ 7))
                || $token = $stream->nextIf(/* Token::NUMBER_TYPE */ 6)
            ) {
                $key = new ConstantExpression($token->getValue(), $token->getLine());
                ...
                throw new SyntaxError(
                    sprintf(
                        ...
                        Token::typeToEnglish($current->getType()),
                        $current->getValue()
        ...
        switch ($name) {
            ...
            case 'block':
                ...
                return new BlockReferenceExpression(
                    $args->getNode(0),
                    \count($args) > 1 ? $args->getNode(1) : null,
                    $line
                );
            case 'attribute':
                $args = $this->parseArguments();
    ...
    public function parseSubscriptExpression($node)
    {
        ...
        if ('.' == $token->getValue()) {
            ...
            if (
                ...
                || (/* Token::OPERATOR_TYPE */ 8 == $token->getType()
                    && preg_match(
                        Lexer::REGEX_NAME,
                        $token->getValue()
                    ))
            ...
            if ($node instanceof NameExpression
                ...
                if (!$arg instanceof ConstantExpression) {
                    throw new SyntaxError(
                        sprintf('Dynamic macro names are not supported (called on "%s").', $node->getAttribute('name')),
                        $token->getLine(),
            ...
            if ($slice) {
                ...
                $filter = new $class(
                    $node,
                    new ConstantExpression('slice', $token->getLine()),
                    $arguments,
                    $token->getLine()
                );
            ...
            if ($namedArguments && $token = $stream->nextIf(/* Token::OPERATOR_TYPE */ 8, '=')) {
                ...
                if ($definition) {
                    ...
                    if (!$this->checkConstantExpression($value)) {
                        throw new SyntaxError(
                            'A default value for an argument must be a constant (a boolean, a string, a number, or an array).',
                            $token->getLine(),
                            $stream->getSourceContext()
                        );
                    }
    ...
    public function parseAssignmentExpression()
    {
        ...
        while (true) {
            ...
            if (\in_array(
                ...
                throw new SyntaxError(
                    sprintf('You cannot assign a value to "%s".', $value),
                    $token->getLine(),
                    $stream->getSourceContext()
                );
            }
    ...
    private function parseTestExpression(Node $node): TestExpression
    {
        ...
        if ('defined' === $name && $node instanceof NameExpression
    ...
    private function getFunctionNodeClass(string $name, int $line): string
    {
        if (!$function = $this->env->getFunction($name)) {
            $e = new SyntaxError(
                sprintf('Unknown "%s" function.', $name),
                $line,
                $this->parser->getStream()->getSourceContext()
            );
