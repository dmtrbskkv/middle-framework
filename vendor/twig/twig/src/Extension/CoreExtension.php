        ...
        public function getTests(): array
        {
            return [
                ...
                new TwigTest(
                    'divisible by',
                    null,
                    ['node_class' => DivisiblebyTest::class, 'one_mandatory_argument' => true]
                ),
...
namespace {
    ...
    function twig_date_converter(Environment $env, $date = null, $timezone = null)
    {
        ...
        if (null === $date || 'now' === $date) {
            ...
            return new \DateTime(
                $date,
                false !== $timezone ? $timezone : $env->getExtension(CoreExtension::class)->getTimezone()
            );
        }

        $asString = (string)$date;
        if (ctype_digit($asString)
            || (!empty($asString) && '-' === $asString[0]
                && ctype_digit(
                    substr($asString, 1)
                ))
        ) {
            ...
            if ($start >= 0 && $length >= 0 && $item instanceof \Iterator) {
                try {
                    return iterator_to_array(
                        new \LimitIterator($item, $start, null === $length ? -1 : $length),
                        $preserveKeys
                    );
...
// a function call is cheaper.
    ...
    function twig_sort_filter($array, $arrow = null)
    {
        if ($array instanceof \Traversable) {
            ...
            throw new RuntimeError(
                sprintf('The sort filter only works with arrays or "Traversable", got "%s".', \gettype($array))
            );
        }
    ...
    function twig_capitalize_string_filter(Environment $env, $string)
    {
        ...
        return mb_strtoupper(mb_substr($string, 0, 1, $charset), $charset) . mb_strtolower(
    ...
    function twig_array_batch($items, $size, $fill = null, $preserveKeys = true)
    {
        if (!twig_test_iterable($items)) {
            throw new RuntimeError(
                sprintf(
                    'The "batch" filter expects an array or "Traversable", got "%s".',
        ...
        if (/* Template::METHOD_CALL */ 'method' !== $type) {
            ...
            if (((\is_array($object) || $object instanceof \ArrayObject)
                    && (isset($object[$arrayItem])
                        || \array_key_exists($arrayItem, (array)$object)))
            ...
            if (/* Template::ARRAY_CALL */ 'array' === $type || !\is_object($object)) {
                ...
                if ($object instanceof ArrayAccess) {
                    ...
                    $message = sprintf(
                        'Impossible to access a key "%s" on an object of class "%s" that does not implement ArrayAccess interface.',
                        $item,
                        \get_class($object)
                    );
    ...
    function twig_array_column($array, $name, $index = null): array
    {
        if ($array instanceof Traversable) {
            ...
            throw new RuntimeError(
                sprintf(
                    'The column filter only works with arrays or "Traversable", got "%s" as first argument.',
                    \gettype($array)
                )
            );
        }
    ...
    function twig_array_filter(Environment $env, $array, $arrow)
    {
        if (!twig_test_iterable($array)) {
            throw new RuntimeError(
                sprintf(
                    'The "filter" filter expects an array or "Traversable", got "%s".',
                    \is_object($array) ? \get_class($array) : \gettype($array)
                )
            );
        }

        if (!$arrow instanceof Closure && $env->hasExtension('\Twig\Extension\SandboxExtension')
            && $env->getExtension(
    ...
    function twig_array_reduce(Environment $env, $array, $arrow, $initial = null)
    {
        ...
        if (!\is_array($array)) {
            if (!$array instanceof \Traversable) {
                throw new RuntimeError(
                    sprintf(
                        'The "reduce" filter only works with arrays or "Traversable", got "%s" as first argument.',
                        \gettype($array)
                    )
                );
            }
