...
abstract class IntegrationTestCase extends TestCase
{
    ...
    public function getTests($name, $legacyTests = false)
    {
        ...
        foreach (
            ...
            if (preg_match(
                '/--TEST--\s*(.*?)\s*(?:--CONDITION--\s*(.*))?\s*(?:--DEPRECATION--\s*(.*?))?\s*((?:--TEMPLATE(?:\(.*?\))?--(?:.*?))+)\s*(?:--DATA--\s*(.*))?\s*--EXCEPTION--\s*(.*)/sx',
                $test,
                $match
            )
            ) {
                ...
                throw new \InvalidArgumentException(
                    sprintf('Test "%s" is not valid.', str_replace($fixturesDir . '/', '', $file))
                );
            }
    ...
    protected function doIntegrationTest(
        ...
        foreach ($outputs as $i => $match) {
            ...
            if (false !== $exception) {
                ...
                $constraintClass = class_exists('PHPUnit\Framework\Constraint\Exception')
                    ? 'PHPUnit\Framework\Constraint\Exception' : 'PHPUnit_Framework_Constraint_Exception';
