...
class Parser
{
    ...
    public function getVarName(): string
    {
        return sprintf(
            '__internal_%s',
            hash('sha256', __METHOD__ . $this->stream->getSourceContext()->getCode() . $this->varNameSalt++)
        );
    }
    ...
    public function parse(TokenStream $stream, $test = null, bool $dropNeedle = false): ModuleNode
    {
        ...
        $node = new ModuleNode(
            new BodyNode([$body]),
            $this->parent,
            new Node($this->blocks),
            new Node($this->macros),
            new Node($this->traits),
            $this->embeddedTemplates,
    ...
    public function getImportedSymbol(string $type, string $alias)
    {
        ...
        return $this->importedSymbols[0][$type][$alias] ??
            ($this->importedSymbols[\count($this->importedSymbols) - 1][$type][$alias] ?? null);
    }
    ...
    private function filterBodyNodes(Node $node, bool $nested = false): ?Node
    {
        ...
        if (
            ...
            || (!$node instanceof TextNode && !$node instanceof BlockReferenceNode
